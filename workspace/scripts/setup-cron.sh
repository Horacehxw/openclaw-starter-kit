...
# ── 1. 每日风险扫描（凌晨 1:00）────────────────────────────
echo ""
echo "🔍 [1/4] 每日风险扫描 (01:00)..."
if openclaw cron add \
    --name "daily-risk-scan" \
    --cron "0 1 * * *" \
    --session isolated \
    --message "执行每日技能风险扫描：1) 按照 scan-all-risk-skill 的指引，扫描 skills/ 下所有已安装 Skill 2) 分析扫描结果：如果发现 CRITICAL 级别风险，检查 HEARTBEAT.md 中是否已有该 Skill 的「待确认」警告，如无则在 HEARTBEAT.md 末尾追加警告块（格式见 AGENTS.md 风险扫描章节），并记录到 .learnings/ERRORS.md；如果发现 HIGH 级别风险，记录到 daily log 建议用户复查；如果无重大风险，在 daily log 记录「扫描完成，无异常」3) 将扫描汇总写入今日 daily log" \
    2>/dev/null; then
    echo "  ✅ daily-risk-scan 已创建"
    SUCCESS=$((SUCCESS + 1))
else
    echo "  ⚠️  daily-risk-scan 创建失败 (可能已存在，用 openclaw cron list 检查)"
    FAIL=$((FAIL + 1))
fi
...
# ── 4. 每周 Skill 巡检（周日 10:00）─────────────────────────
echo ""
echo "🔧 [4/4] 每周 Skill 巡检 (周日 10:00)..."
if openclaw cron add \
...
echo ""
echo "手动触发测试:"
echo "  openclaw cron run daily-risk-scan"
